...
def test_route_returns_default_when_settings_not_loaded(router):
    assert router.route("approach_explorer") == "mistralai/mistral-7b-instruct:free"


def test_route_returns_model_from_settings(router):
    router.load_settings({"roles": {"approach_explorer": {"model": "anthropic/claude-opus-4.7"}}})
    assert router.route("approach_explorer") == "anthropic/claude-opus-4.7"
...
def test_settings_model_overrides_default(router):
    router.load_settings({"roles": {"approach_explorer": {"model": "google/gemini-2.5-flash"}}})
    assert router.route("approach_explorer") == "google/gemini-2.5-flash"
